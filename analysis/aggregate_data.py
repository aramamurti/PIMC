...
def aggregate_data(folder_path, trial, output_folder):
    ...
    if len(num_temps) == 1:
        ...
        we = open(folder_path+output_folder+'energy_data_{}.csv'.format(trial),'w')
        cwe = csv.writer(we)
        cwe.writerow(temps)
        cwe.writerow(enlist)
        cwe.writerow(stdenlist)

        wp = open(folder_path+output_folder+'permutation_data_{}.csv'.format(trial),'w')

        cwp = csv.writer(wp)
        for i in range(len(temps)):
            cwp.writerow([temps[i]]+permlist[i])

        ww = open(folder_path+output_folder+'winding_data_{}.csv'.format(trial),'w')
        cww = csv.writer(ww)
        for i in range(len(temps)):
            cww.writerow([temps[i]]+wn[i].tolist())


        wf = open(folder_path+output_folder+'free_energy_data_{}.csv'.format(trial),'w')
        cwf = csv.writer(wf)
        cwf.writerow(temps)
        cwf.writerow(dFx)
        cwf.writerow(dFy)

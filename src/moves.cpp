...
void Moves::attempt(int &id, Parameters &params, Paths &paths, RNG &rng){
}
...
void Center_of_Mass::attempt(int &id,Parameters &params, Paths &paths, RNG &rng){
    Moves::attempt(id, params, paths, rng);
    if(params.particles == 0) return;
            ...
            if(!(ptcls[j] == params.worm_head.second && i+params.my_start < params.worm_head.first) && !(ptcls[j] == params.worm_tail.second && i+params.my_start > params.worm_tail.first)){
                switch(params.potential){
                        ...
                        for(int ptcl = 0; ptcl < params.particles; ++ptcl){
                            if(!(ptcl == params.worm_head.second && i+params.my_start < params.worm_head.first) && !(ptcl == params.worm_tail.second && i+params.my_start > params.worm_tail.first)){
                                if(std::find(ptcls.begin(), ptcls.end(), ptcl) == ptcls.end()){
                                    old_action_p[i] += potential_value_aziz(paths.get_separation(i, ptcl, ptcls[j]));
                                    ...
                                    new_action_p[i] += potential_value_aziz(r);
                    ...
                    case 2:
                        for(int ptcl = 0; ptcl < params.particles; ++ptcl){
                            if(!(ptcl == params.worm_head.second && i+params.my_start < params.worm_head.first) && !(ptcl == params.worm_tail.second && i+params.my_start > params.worm_tail.first)){
                                old_action_p[i] += potential_value_coulomb(paths.get_separation_vector(i, ptcl, ptcls[j]), kVec, paths.charge[ptcl],paths.charge[ptcls[j]],  params);
                                distance(paths.get_coordinate(i,ptcl), new_coordinates[j*params.slices_per_process+i], dist, params.box_size);
                                double r = sqrt(std::inner_product(dist.begin(), dist.end(), dist.begin(), 0.0));
                                new_action_p[i] += potential_value_coulomb(dist, kVec,  paths.charge[ptcl],paths.charge[ptcls[j]], params);
                                new_distances.push_back(std::tuple<std::pair<int,int>,std::vector<double>,double>(std::pair<int,int>(paths.get_coordinate_key(i, ptcl), paths.get_coordinate_key(i, ptcls[j])), dist, r));
                            }
                        }
                        break;
...
Bisection::Bisection(int& id, Parameters &params, MPI_Comm &local) : Moves(local){
    multistep_dist = pow(2, floor(log2(params.total_slices/5.)));
...
void Bisection::attempt(int &id,Parameters &params, Paths &paths, RNG &rng){
    Moves::attempt(id, params, paths, rng);
    if(params.particles == 0) return;
    ...
    if(ac_re){
        ...
        for(int i = bisection_info[0]; i <= bisection_info[1]; ++i)
            if(i%params.total_slices >= params.my_start && i%params.total_slices <= params.my_end){
                put_in_box(new_coordinates[i%params.slices_per_process], params.box_size);
                paths.set_coordinate(i%params.slices_per_process, ptcl_slice[i%params.slices_per_process],new_coordinates[i%params.slices_per_process], false);
            }
        paths.update_separations(new_distances);
    }
    bisection_info.clear();
        ...
        if(slice >= params.my_start && slice <= params.my_end){
            switch(params.potential){
                    ...
                    for(int ptcl = 0; ptcl < params.particles; ++ptcl){
                        if(!(ptcl == params.worm_head.second && slice < params.worm_head.first) && !(ptcl == params.worm_tail.second && slice > params.worm_tail.first)){
                            if(ptcl != ptcl_slice[slice%params.slices_per_process]){
                                old_action_p[slice%params.slices_per_process] += potential_value_aziz(paths.get_separation(slice%params.slices_per_process, ptcl, ptcl_slice[slice%params.slices_per_process]));
                                ...
                                double r = sqrt(std::inner_product(dist.begin(), dist.end(), dist.begin(), 0.0));
                                new_action_p[slice%params.slices_per_process] += potential_value_aziz(r);
                ...
                case 2:
                    for(int ptcl = 0; ptcl < params.particles; ++ptcl){
                        if(!(ptcl == params.worm_head.second && slice < params.worm_head.first) && !(ptcl == params.worm_tail.second && slice > params.worm_tail.first)){
                            old_action_p[slice%params.slices_per_process] += potential_value_coulomb(paths.get_separation_vector(slice%params.slices_per_process, ptcl, ptcl_slice[slice%params.slices_per_process]), kVec,  paths.charge[ptcl],paths.charge[ptcl_slice[slice%params.slices_per_process]], params);
                            distance(paths.get_coordinate(slice%params.slices_per_process,ptcl), new_coordinates[slice%params.slices_per_process], dist, params.box_size);
                            double r = sqrt(std::inner_product(dist.begin(), dist.end(), dist.begin(), 0.0));
                            new_action_p[slice%params.slices_per_process] += potential_value_coulomb(dist, kVec, paths.charge[ptcl], paths.charge[ptcl_slice[slice%params.slices_per_process]],  params);
                            new_distances.push_back(std::tuple<std::pair<int,int>,std::vector<double>,double>(std::pair<int,int>(paths.get_coordinate_key(slice%params.slices_per_process, ptcl),paths.get_coordinate_key(slice%params.slices_per_process, ptcl_slice[slice%params.slices_per_process])), dist, r));
                        }
                    }
                    break;
...
void Open::attempt(int &id, Parameters &params, Paths &paths, RNG &rng){
            ...
            switch(params.potential){
                ...
                case 1:
                    for(int ptcl = 0; ptcl < params.particles; ++ptcl){
                        if(!(ptcl == params.worm_head.second && slice < params.worm_head.first) && !(ptcl == params.worm_tail.second && slice > params.worm_tail.first)){
                            if(ptcl != part){
                                old_action_p[slice%params.slices_per_process] += potential_value_aziz(paths.get_separation(slice%params.slices_per_process, ptcl, part));
                            }
                        }
                    }
                    break;
                case 2:
                    for(int ptcl = 0; ptcl < params.particles; ++ptcl){
                        if(!(ptcl == params.worm_head.second && slice < params.worm_head.first) && !(ptcl == params.worm_tail.second && slice > params.worm_tail.first)){
                            old_action_p[slice%params.slices_per_process] += potential_value_coulomb(paths.get_separation_vector(slice%params.slices_per_process,  ptcl, part), kVec, paths.charge[ptcl], paths.charge[part], params);
                        }
                    }
                    break;
            }
...
void Close::attempt(int &id, Parameters &params, Paths &paths, RNG &rng){
    if(!params.worm_on) return;
    int distance = (params.worm_head.first - params.worm_tail.first + params.total_slices)%params.total_slices;
    if(distance > params.Mbar + 1 || distance == 0) return;
    Moves::attempt(id, params, paths, rng);
    new_coordinates.resize(params.slices_per_process);
                ...
                case 2:
                    for(int ptcl = 0; ptcl < params.particles; ++ptcl){
                        if(!(ptcl == params.worm_head.second && slice <= params.worm_head.first) && !(ptcl == params.worm_tail.second && slice >= params.worm_tail.first)){
                            distance(paths.get_coordinate(slice%params.slices_per_process,ptcl), new_coordinates[slice%params.slices_per_process], dist, params.box_size);
                            double r = sqrt(std::inner_product(dist.begin(), dist.end(), dist.begin(), 0.0));
                            new_action_p[slice%params.slices_per_process] += potential_value_coulomb(dist, kVec, paths.charge[ptcl],paths.charge[params.worm_tail.second], params);
                            new_distances[slice%params.slices_per_process].push_back(std::tuple<int,std::vector<double>,double>(paths.get_coordinate_key(slice%params.slices_per_process, ptcl), dist, r));
                        }
                    }
...
void Insert::attempt(int &id, Parameters &params, Paths &paths, RNG &rng){
    if(params.worm_on) return;
    Moves::attempt(id, params, paths, rng);
                ...
                case 2:
                    for(int ptcl = 0; ptcl < params.particles; ++ptcl){
                        if(!(ptcl == params.worm_head.second && slice < params.worm_head.first) && !(ptcl == params.worm_tail.second && slice > params.worm_tail.first)){
                            distance(paths.get_coordinate(slice%params.slices_per_process,ptcl),new_coordinates[slice%params.slices_per_process], dist, params.box_size);
                            double r =sqrt(std::inner_product(dist.begin(), dist.end(), dist.begin(), 0.0));
                            new_action_p[slice%params.slices_per_process] += potential_value_coulomb(dist, kVec, paths.charge[ptcl], insert_info[2], params);
                            new_distances[slice%params.slices_per_process].push_back(std::tuple<int,std::vector<double>,double>(paths.get_coordinate_key(slice%params.slices_per_process, ptcl), dist, r));
                        }
                    }
                    dist = std::vector<double>(params.dimensions,0);
...
void Remove::attempt(int &id, Parameters &params, Paths &paths, RNG &rng){
    if(!params.worm_on) return;
    if(params.worm_length > params.Mbar + 1) return;
    Moves::attempt(id, params, paths, rng);
    check(id, params, paths, rng);
    MPI_Bcast(&ac_re, 1, MPI_INT,0, local_comm);
    ++num_attempts;
            ...
            switch(params.potential){
                ...
                case 1:
                    for(int ptcl = 0; ptcl < params.particles; ++ptcl){
                        if(!(paths.broken[ptcl])){
                            if(ptcl != column){
                                old_action_p[slice%params.slices_per_process] += potential_value_aziz(paths.get_separation(slice%params.slices_per_process, ptcl, column));
                            }
                        }
                    }
                    break;
                case 2:
                    for(int ptcl = 0; ptcl < params.particles; ++ptcl){
                        if(!(ptcl == params.worm_head.second && slice < params.worm_head.first) && !(ptcl == params.worm_tail.second && slice > params.worm_tail.first)){
                            old_action_p[slice%params.slices_per_process] += potential_value_coulomb(paths.get_separation_vector(slice%params.slices_per_process, ptcl, column), kVec, paths.charge[ptcl], paths.charge[column], params);
                        }
                    }
                    break;
            }
...
void Advance_Tail::attempt(int &id, Parameters &params, Paths &paths, RNG &rng){
    if(!params.worm_on) return;
    Moves::attempt(id, params, paths, rng);
...
void Advance_Head::attempt(int &id, Parameters &params, Paths &paths, RNG &rng){
    if(!params.worm_on) return;
    Moves::attempt(id, params, paths, rng);
    M = rng.randint(params.Mbar)+1;
    MPI_Bcast(&M, 1, MPI_INT, 0, local_comm);
                ...
                case 2:
                    for(int ptcl = 0; ptcl < params.particles; ++ptcl){
                        if(!(ptcl == params.worm_head.second && slice <= params.worm_head.first) && !(ptcl == params.worm_tail.second && slice > params.worm_tail.first)){
                            distance(paths.get_coordinate(slice%params.slices_per_process,ptcl), new_coordinates[slice%params.slices_per_process], dist, params.box_size);
                            double r = sqrt(std::inner_product(dist.begin(), dist.end(), dist.begin(), 0.0));
                            new_action_p[slice%params.slices_per_process] += potential_value_coulomb(dist, kVec, paths.charge[ptcl], paths.charge[params.worm_head.second], params);
                            new_distances[slice%params.slices_per_process].push_back(std::tuple<int,std::vector<double>,double>(paths.get_coordinate_key(slice%params.slices_per_process, ptcl), dist, r));
                        }
                    }
                    dist = std::vector<double>(params.dimensions,0);
...
void Recede_Head::attempt(int &id, Parameters &params, Paths &paths, RNG &rng){
    if(!params.worm_on) return;
    Moves::attempt(id, params, paths, rng);
    M = rng.randint(params.Mbar)+1;
            ...
            switch(params.potential){
                ...
                case 1:
                    for(int ptcl = 0; ptcl < params.particles; ++ptcl){
                        if(!(ptcl == params.worm_head.second && slice < params.worm_head.first) && !(ptcl == params.worm_tail.second && slice > params.worm_tail.first)){
                            if(ptcl != col){
                                old_action_p[slice%params.slices_per_process] += potential_value_aziz(paths.get_separation(slice%params.slices_per_process, ptcl, col));
                            }
                        }
                    }
                    break;
                case 2:
                    for(int ptcl = 0; ptcl < params.particles; ++ptcl){
                        if(!(ptcl == params.worm_head.second && slice < params.worm_head.first) && !(ptcl == params.worm_tail.second && slice > params.worm_tail.first)){
                            old_action_p[slice%params.slices_per_process] += potential_value_coulomb(paths.get_separation_vector(slice%params.slices_per_process, ptcl, col), kVec, paths.charge[ptcl], paths.charge[col], params);
                        }
                    }
                    break;
            }
...
void Recede_Tail::attempt(int &id, Parameters &params, Paths &paths, RNG &rng){
    if(!params.worm_on) return;
    Moves::attempt(id, params, paths, rng);
    M = rng.randint(params.Mbar)+1;
...
void Swap_Tail::attempt(int &id, Parameters &params, Paths &paths, RNG &rng){
    if(!params.worm_on) return;
    Moves::attempt(id, params, paths, rng);
    int start_slice = params.worm_tail.first;
        ...
        if(swap_slice >= params.my_start && swap_slice <= params.my_end){
            ...
            if(!possible_swaps.empty()){
                for(int poss = possible_swaps.size() - 1; poss >= 0; --poss){
                    if(paths.charge[poss] != paths.charge[params.worm_tail.second])
            ...
            switch(params.potential){
                ...
                case 1:
                {
                    for(int ptcl = 0; ptcl < params.particles; ++ptcl){
                        if(!(ptcl == params.worm_head.second && slice < params.worm_head.first) && !(ptcl == params.worm_tail.second && slice > params.worm_tail.first)){
                            if(i != 0 && ptcl != ptcl1){
                                old_action_p[slice%params.slices_per_process] += potential_value_aziz(paths.get_separation(slice%params.slices_per_process, ptcl, ptcl1));
                                distance(paths.get_coordinate(slice%params.slices_per_process,ptcl), new_coordinates[slice%params.slices_per_process], dist, params.box_size);
                                double r = sqrt(std::inner_product(dist.begin(), dist.end(), dist.begin(), 0.0));
                                new_action_p[slice%params.slices_per_process] += potential_value_aziz(r);
                ...
                case 2:
                {
                    for(int ptcl = 0; ptcl < params.particles; ++ptcl){
                        if(!(ptcl == params.worm_head.second && slice < params.worm_head.first) && !(ptcl == params.worm_tail.second && slice > params.worm_tail.first)){
                            old_action_p[slice%params.slices_per_process] += potential_value_coulomb(paths.get_separation_vector(slice%params.slices_per_process, ptcl, ptcl1), kVec, paths.charge[ptcl], paths.charge[ptcl1], params);
                            distance(paths.get_coordinate(slice%params.slices_per_process,ptcl), new_coordinates[slice%params.slices_per_process], dist, params.box_size);
                            double r = sqrt(std::inner_product(dist.begin(), dist.end(), dist.begin(), 0.0));
                            new_action_p[slice%params.slices_per_process] += potential_value_coulomb(dist, kVec, paths.charge[ptcl], paths.charge[ptcl1], params);
                            new_distances.push_back(std::tuple<std::pair<int,int>,std::vector<double>,double>(std::pair<int,int>(paths.get_coordinate_key(slice%params.slices_per_process, ptcl),paths.get_coordinate_key(slice%params.slices_per_process, ptcl1)),  dist, r));
                        }
                    }
                    if(i != 0 && i != params.Mbar){
                        dist = std::vector<double>(params.dimensions, 0);
                        new_action_p[slice%params.slices_per_process] += potential_value_coulomb(dist, kVec, paths.charge[ptcl1], paths.charge[ptcl1], params);
                    }
                    break;
                }
            }
...
void Swap_Head::attempt(int &id, Parameters &params, Paths &paths, RNG &rng){
    if(!params.worm_on) return;
    Moves::attempt(id, params, paths, rng);
    int start_slice = params.worm_head.first;
                        ...
                        if(!(ptcl == params.worm_head.second && slice < params.worm_head.first) && !(ptcl == params.worm_tail.second && slice > params.worm_tail.first)){
                            if(i != 0 && ptcl != ptcl1){
                                ...
                                if(ptcl != params.worm_head.second)
                                    new_action_p[slice%params.slices_per_process] += potential_value_aziz(sqrt(std::inner_product(dist.begin(), dist.end(), dist.begin(), 0.0)));
                            }
                        }
                    }
                    break;

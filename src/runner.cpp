...
void Runner::equilibrate(int &id, Parameters &params, Paths &paths, RNG &rng){
    ...
    for(int step = 0; step < params.equilibration; ++step){
        if(id == 0)
            std::cout << "Equilibration step " <<step << "/" <<params.equilibration << ":\t" << params.real_particles << " particles" << std::endl;
        if(double(step)/params.equilibration < 1/3.){
            for(int j = 0; j < params.particles; ++j){
                for(int n = 0; n < params.particles; ++n){
                    ...
                    moves[choice].attempt(id, params, paths, rng);
...
//                    std::cout << "[Step " << step << " Center of Mass Equilibration]\t Acceptance : " << com_acc_rat << ", Delta : " << moves[0].get_delta() << std::endl;
            ...
            for(int j = 0; j < std::max(params.particles,params.init_particles); ++j){
                ...
                if(off_diag_att_ctr == off_diag_att){
                    ...
                    off_diag_att_ctr = 0;
//                std::cout <<"[Step " << step << " Chemical Potential Equilibration]\t Average Particles : " << avg_part << ", µ : " << params.mu << std::endl;
                }
                if(conf_counter == conf_att){
                    ...
                    conf_counter = 0;
                    diag_counter = 0;
//                    std::cout <<"[Step " << step << " Worm Constant Equilibration]\t Diagonal Fraction : " << diag_frac << ", C0 : " << params.C0 << std::endl;
                }
            }
        }
    }
...
void Runner::run(int &id, Parameters &params, IO &writer){
    ...
    for(int step = 0; step < params.end; ++step){
        if(id == 0)
            std::cout << "Simulation step " << step << "/" <<params.end << ":\t" << params.real_particles << " particles" << std::endl;
        for(int j = 0; j < std::max(params.particles,params.init_particles); ++j){
            for(int n = 0; n < std::max(params.particles,params.init_particles); ++n){
                int choice = 0;
                if(params.worm_on)

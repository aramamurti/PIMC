...
:multvec{1.0,20.0,100.0, 400.0}{ // Multiplication factor for the swap probabilities
    ...
    if(params->is_boson()){
        ...
        probList.resize(params->get_num_timeslices());
    ...
    inline float Path::vext(int slice, int ptcl){
        ...
        if(params->get_potentials()[1]){    //LJ
            for(int i = 0; i < params->get_num_particles(); i++){
                if(i != ptcl){
                    vectorf distvec =ute->dist(beads->getPairSS(ptcl, i, slice), params->get_box_size());
    ...
    float Path::kineticAction(int slice, int dist){
        ...
        for(int ptcl = 0; ptcl < params->get_num_particles(); ptcl++){
            ...
            vectorf distVec = ute->dist(pair, params->get_box_size());
    ...
    void Path::put_in_box(){
        if(params->get_box_size() != -1)
            for(int ptcl = 0; ptcl < params->get_num_particles(); ptcl++)
                for(int slice = 0; slice < params->get_num_timeslices(); slice++){
                    beads->setOne(ptcl, slice, ute->location(beads->getOne(ptcl, slice), params->get_box_size()));
                }
